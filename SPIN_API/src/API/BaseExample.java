 ...
 * 
 * 
 *  A simple code to create a SPIN function with 3 variables, 1 Constants and 2 Usage, using the TopBraid Composer API and then call it to get the dynamic result
 *  Used formula = (U + U) * C -> Heroku use case
 *  It also lists the functions variables, it's usage variables and requests to the user to insert a value for each usage variable. 
 *  ----------------------------------------------------------------------------------------
...
public class BaseExample {
	...
	private static final String QUERY =
		"SELECT ?result\n" +
		"WHERE {\n" +
		"    :webhours price:hasValue ?v .\n" +
		"    ?v gr:hasValue ?value1 . \n " +
		"    :workerhours price:hasValue ?v2 . \n " +
		"    ?v2 gr:hasValue ?value2 . \n" +
		"    :cost price:hasValue ?v3 . \n" +
...
private static Function createFunction(Model model) {
		...
		//create a variable
		Resource variable = model.createResource(BASE + "cost").
				...
				addProperty(model.createProperty(PRICE+"hasValue"),
						model.createResource(GR+"QuantitativeValue"+ i++).addProperty(model.createProperty(GR+"hasUnitOfMeasurement"),model.createTypedLiteral("MON",XSDDatatype.XSDstring)).
						addProperty(model.createProperty(GR+"hasValue"),model.createTypedLiteral(new Float(0.05),XSDDatatype.XSDfloat)));

		//create a second variable
		Resource variable2 = model.createResource(BASE + "workerhours").
				addProperty(model.createProperty(RDFNS+"type"),model.createResource(PRICE + "Usage"));


		Resource variable3 = model.createResource(BASE + "webhours").
				addProperty(model.createProperty(RDFNS+"type"),model.createResource(PRICE + "Usage"));
