...
public class SPINConstraints {
	...
	private static void addConstraintViolations(List<ConstraintViolation> results, SPINInstance instance, Property predicate, boolean matchValue, List<SPINStatistics> stats, ProgressMonitor monitor) {
	...
	private static synchronized Query convertAskToConstruct(Query ask, org.topbraid.spin.model.Query spinQuery, String label) {
		...
		try {
			...
			if(label != null) {
				bgp.add(Triple.create(subject, RDFS.label.asNode(), NodeFactory.createLiteral(label)));
			}
	...
	private static void run(Model model, List<ConstraintViolation> results, List<SPINStatistics> stats, ProgressMonitor monitor) {
		...
		Map<Resource,List<CommandWrapper>> class2Query = SPINQueryFinder.getClass2QueryMap(model, model, SPIN.constraint, true, true);
